import {
  getHomeDisciplines,
  AddHomeDisciplineModal,
  TableHomeDisciplines,
} from "@/modules/cms/home-disciplines";
...
export default async function HomeDisciplinesPage() {
  ...
  // Stats calculations
  const activeCount = homeDisciplinesData.filter((b) => b.isActive).length;
  const inactiveCount = homeDisciplinesData.filter((b) => !b.isActive).length;
  const totalCount = homeDisciplinesData.length;
  ...
  return (
    <>
      ...
      <div className="flex flex-col space-y-3 mt-8">
        <TableHomeDisciplines homeDisciplines={homeDisciplinesData} />
      </div>
    </>
  );
}
