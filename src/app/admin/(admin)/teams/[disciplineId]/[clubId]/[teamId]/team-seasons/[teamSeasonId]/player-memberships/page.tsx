      ...
      <div className="flex flex-col gap-6 page-content">
        <Card className="p-6 shadow-[0px_4px_12px_rgba(0,0,0,0.06)] border border-border flex flex-col gap-5 bg-surface-container-lowest">
          <div className="flex items-center justify-between">
            <h3 className="font-headline font-bold text-lg">Resumen Financiero y Reglas</h3>
            <Chip color="accent" variant="soft" size="sm" className="font-semibold tracking-wide uppercase">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div>
              <p className="text-xs text-muted-foreground mb-1 uppercase font-semibold">Modelo de Cobro</p>
            ...
            {teamSeason.billingType !== "SINGLE_ONLY" && (
              <>
                <div>
                  <p className="text-xs text-muted-foreground mb-1 uppercase font-semibold">Matrícula</p>
                  <p className="font-bold text-sm">{teamSeason.registrationFee ? `${teamSeason.registrationFee} Bs.` : "Gratis"}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-1 uppercase font-semibold">Cuota Base ({
                    teamSeason.billingFrequency === "WEEKLY" ? "Semanal" : 
                    teamSeason.billingFrequency === "BIWEEKLY" ? "Quincenal" : "Mensual"
                  })</p>
                  <p className="font-bold text-sm">{teamSeason.recurringFee ? `${teamSeason.recurringFee} Bs.` : "Gratis"}</p>
                </div>
              </>
            )}
            {(teamSeason.billingType === "SINGLE_ONLY" || teamSeason.billingType === "BOTH") && (
              <div>
                <p className="text-xs text-muted-foreground mb-1 uppercase font-semibold">Tarifa Temporada Completa</p>
                <p className="font-bold text-sm">{teamSeason.seasonFee ? `${teamSeason.seasonFee} Bs.` : "Gratis"}</p>
              </div>
            )}
            <div className="col-span-full">
               <p className="text-xs text-muted-foreground mb-2 uppercase font-semibold">Opciones de Prorrateo Activas</p>
               <div className="flex flex-wrap gap-2">
                 {teamSeason.prorateRegistrationFee && <Chip size="sm" variant="soft" color="default">Matrícula</Chip>}
                 {teamSeason.prorateFirstRecurringFee && <Chip size="sm" variant="soft" color="default">Primer Cargo Recurrente</Chip>}
                 {teamSeason.prorateLastRecurringFee && <Chip size="sm" variant="soft" color="default">Último Cargo Recurrente</Chip>}
                 {teamSeason.prorateSeasonFee && <Chip size="sm" variant="soft" color="default">Tarifa Temporada (Pago Único)</Chip>}
                 {!teamSeason.prorateRegistrationFee && !teamSeason.prorateFirstRecurringFee && !teamSeason.prorateLastRecurringFee && !teamSeason.prorateSeasonFee && (
                   <span className="text-xs italic text-muted-foreground">Ninguna (Se cobran los montos completos siempre)</span>
                 )}
               </div>
            </div>
          </div>
