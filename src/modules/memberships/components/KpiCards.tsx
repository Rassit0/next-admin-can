...
export const KpiCards = ({ data }: KpiCardsProps) => {
  ...
  const kpis = [
    ...
    },
    {
      ...
      icon: (
        <HugeiconsIcon icon={Alert01Icon} size={24} className="text-danger" />
      ),
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {kpis.map((kpi, index) => (
        <Card
          key={index}
          className="border-none bg-background/60 dark:bg-default-100/50 shadow-sm p-6"
        >
          ...
          <div className="mt-4 flex items-center gap-2">
            {kpi.trend !== undefined && (
              <span
                className={`text-xs font-semibold ${kpi.trend >= 0 ? "text-success" : "text-danger"}`}
              >
                {kpi.trend >= 0 ? "+" : ""}
                {kpi.trend}%
              </span>
