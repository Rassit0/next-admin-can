...
export const DirectTransactionDrawer = ({
  ...
  const handleSubmit = async () => {
    ...
    if (isSplitPayment) {
      if (splits.length === 0) {
        toast.error("Debe existir al menos una distribuciiÂ³n");
        return;
      }
      let totalSplits = 0;
      ...
      if (
        Number(totalSplits.toFixed(2)) !== Number(Number(amount).toFixed(2))
      ) {
        toast.error(
          `La suma de las distribuciones (${totalSplits}) no coincide con el monto total (${amount})`,
        );
        return;
      }
    } else {
    ...
    try {
      ...
      toast.error("OcurriiÂ³ un error inesperado");
    } finally {
      setIsLoading(false);
    }
  };
                      ...
                      setSplits([
                        {
                          ...
                          amount: amount || "",
                          paymentMethod:
                            defaultAcc?.allowedPaymentMethods?.[0] || "CASH",
                          financialAccountId: defaultAcc ? defaultAcc.id : "",
                          reference: "",
                        },
                      ]);
