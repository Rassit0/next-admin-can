  ...
  return (
    <Table>
      <Table.ScrollContainer>
        <Table.Content aria-label="Historial de Flujo de Caja" className="min-w-200">
          <Table.Header>
            <Table.Column allowsSorting id="transactionDate" isRowHeader>
              <SortableColumnHeader id="transactionDate">Fecha</SortableColumnHeader>
            </Table.Column>
            <Table.Column allowsSorting id="concept">
              <SortableColumnHeader id="concept">Concepto</SortableColumnHeader>
            </Table.Column>
            ...
            <Table.Column allowsSorting id="paymentMethod">
              <SortableColumnHeader id="paymentMethod">Método</SortableColumnHeader>
            </Table.Column>
            <Table.Column>Origen</Table.Column>
            <Table.Column>Cuenta Financiera</Table.Column>
            <Table.Column allowsSorting id="amount">
              <div className="text-right">
                <SortableColumnHeader id="amount" className="justify-end">Monto</SortableColumnHeader>
              </div>
            </Table.Column>
          </Table.Header>
          <Table.Body
            renderEmptyState={() => (
              <div className="flex h-full w-full flex-col items-center justify-center gap-4 p-8 text-center">
                ...
                <div className="flex flex-col gap-1">
                  <span className="text-medium font-medium">No se encontraron movimientos</span>
              ...
              <Table.Row key={transaction.id} id={transaction.id}>
                <Table.Cell>
                  <span className="whitespace-nowrap text-default-600">
                    {new Date(transaction.transactionDate).toLocaleString("es-BO")}
                  </span>
                </Table.Cell>
                <Table.Cell>
                  <span className="font-medium max-w-[250px] truncate block" title={transaction.concept}>
                    {transaction.concept}
                  </span>
                </Table.Cell>
                <Table.Cell>
                ...
                <Table.Cell>
                  <span className="text-default-600 font-medium">
                    {transaction.financialAccountName || <span className="text-default-400 italic">Sin asignar</span>}
                  </span>
                </Table.Cell>
                <Table.Cell>
