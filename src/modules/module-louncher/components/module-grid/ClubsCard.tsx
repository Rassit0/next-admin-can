...
import { ArrowRight02Icon, Flag03Icon } from "@hugeicons/core-free-icons";
import { HugeiconsIcon } from "@hugeicons/react";
import Link from "next/link";
import React from "react";

export const ClubsCard = async () => {
  ...
  return (
    <Link href={`/admin/clubs/dashboard`}>
      <Card
        className="
        group
        p-8
      ...
      >
        <div className="flex items-start justify-between mb-6">
          ...
          <Chip className="px-3 py-1 bg-default text-default-foreground rounded-full text-xs font-bold backdrop-blur-sm">
            {clubsOptionsResponse.data.data.length} disponibles
          </Chip>
        </div>
