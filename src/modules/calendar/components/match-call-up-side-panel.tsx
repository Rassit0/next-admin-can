...
import { IMatchCallUpCandidate, updateMatchCallUpSide, IMatchCallUpsResponse } from "../actions/match-call-ups.action";
...
export const MatchCallUpSidePanel = ({
  ...
  const needsInitialSave = configuredAt === null && hasCategory && candidates.length > 0;
  ...
  useEffect(() => {
    ...
    if (configuredAt === null && hasCategory) {
      ...
      // Si ya fue configurado (incluso vacío explícitamente), respetar lo que viene del backend
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.error || !res.data) {
        ...
        toast.success(`Convocatoria ${side === "HOME" ? "local" : "visitante"} actualizada`);
        onDirtyChange(false);
      ...
      {!hasCategory ? (
        <div className="bg-warning/10 p-4 rounded-lg flex flex-col items-center justify-center text-center h-48 border border-warning/20">
          <p className="text-sm text-warning font-semibold">
            Este equipo no tiene una categoría administrada para gestionar convocados.
          </p>
        </div>
      ) : candidates.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-center h-48 text-muted">
          <p className="text-sm">No hay jugadores elegibles para este partido.</p>
        </div>
      ) : (
        <>
          <div className="flex flex-col gap-3 flex-1 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto pr-2 space-y-2 pb-16">
              {filteredCandidates.map((candidate) => {
                ...
                const isInOpposite = oppositeCallUps.some((c) => c.playerId === candidate.playerId);
                const isConflict = isSelected && isInOpposite;
                const isDisabled = !canUpdateMatches || (isInOpposite && !isSelected);

                return (
                  <div
                    key={candidate.playerId}
                    className={`flex items-center justify-between p-2 rounded-md transition-colors ${
                      isDisabled ? "opacity-60 bg-default-50" : "hover:bg-default-100 cursor-pointer"
