...
export const AccountingAlerts = ({ alerts }: Props) => {
  ...
  const renderAlert = (alert: IDashboardAlert, index: number) => {
    return (
      <div key={`${alert.context}-${index}`} className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between p-3 rounded-lg hover:bg-default-100 transition-colors border border-transparent hover:border-default-200">
        <div className="flex items-start gap-3">
          <div className={`p-2 rounded-lg bg-${alert.severity}/10 text-${alert.severity}`}>
            {alert.severity === "danger" ? <HugeiconsIcon icon={Alert02Icon} size={20} /> : <HugeiconsIcon icon={CalendarMinus02Icon} size={20} />}
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <Chip size="sm" color={alert.severity} className="h-5 text-[10px]">
                {alert.count} cuentas
              </Chip>
            </div>
          </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div className="border border-default-200 rounded-large shadow-sm bg-default-50 flex flex-col">
          ...
          <div className="p-2 flex flex-col gap-1 max-h-[320px] overflow-y-auto scrollbar-hide">
