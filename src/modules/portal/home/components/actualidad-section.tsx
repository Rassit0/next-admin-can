          ...
          {news.map((item, idx) => (
            ...
            >
              <Link href={`/present/${item.slug}`} className="block h-full">
                <TiltCard className="group relative h-96 w-full overflow-hidden rounded-lg border border-border bg-card shadow-neon-soft transition-shadow hover:shadow-neon">
                  {item.imageUrl ? (
                    <Image
                      src={item.imageUrl}
                      alt={item.title}
