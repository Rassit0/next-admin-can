...
export const FormCourseSeason = ({
  ...
  // form params
  const [name, setName] = useState<string>(courseSeason?.name || "Regular");
  ...
  const handleShiftChange = useCallback(
    (index: number, field: keyof IShiftConfigForm, value: any) => {
      setShifts((prev) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement> | null) => {
      ...
      if (shifts.length === 0) {
        ...
        shifts.forEach((shift, index) => {
          ...
          const isDuplicate = shifts.some(
            (s, i) =>
              i !== index && s.shiftId === shift.shiftId && s.shiftId !== "",
          );
          if (isDuplicate) {
            newErrors[`shift_${index}_shiftId`] =
              "Este turno ya fue agregado en otro bloque";
          }
        });
      }
    }
    ...
    if (courseSeason && !isClone) {
      ...
      toast.success("ÃÂ¡Cambios guardados!", {
        description: "La temporada se ha actualizado correctamente.",
      });
