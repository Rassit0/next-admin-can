...
import { Table, Button, Card, Chip } from "@heroui/react";
...
export const InternalTransfersClient = ({ transfers, accounts }: Props) => {
  ...
  const handleCancel = async (id: string) => {
    if (
      !confirm(
        "ÃÂ¿EstiÂ¡ seguro de anular esta transferencia? Esto revertiriÂ¡ los saldos de ambas cuentas.",
      )
    ) {
      return;
    }
      ...
      <Card>
        <Table aria-label="Tabla de transferencias internas">
          <Table.ScrollContainer>
            <Table.Content
              aria-label="Tabla de transferencias internas"
              className="w-full"
            >
              <Table.Header>
                  ...
                  <Table.Row key={transfer.id}>
                    ...
                    <Table.Cell>
                      <span className="text-danger font-medium">
                        {transfer.sourceTransaction?.financialAccount?.name ||
                          "Desconocido"}
                      </span>
                    </Table.Cell>
                    <Table.Cell>
                      <span className="text-success font-medium">
                        {transfer.destinationTransaction?.financialAccount
                          ?.name || "Desconocido"}
                      </span>
                    </Table.Cell>
                    <Table.Cell>
