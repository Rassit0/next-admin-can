    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-4">
        {filteredFixtures.map((fixture, idx) => {
          ...
          const awayTeamInitial = fixture.awayTeam.name.substring(0, 3);
          // Dicebear generated avatar URL for fallback
          const awayTeamAvatar = `https://api.dicebear.com/9.x/initials/svg?seed=${awayTeamInitial}`;
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between gap-4 px-4">
                <div className="flex flex-1 flex-col items-center gap-2">
                  <div className="relative flex h-16 w-16 items-center justify-center rounded-full bg-white p-2 shadow-sm">
                    {fixture.homeTeam.imageUrl ? (
                      ...
                      <span className="font-heading text-xl font-bold">{fixture.homeTeam.name.substring(0, 3)}</span>
                    )}
                  </div>
                ...
                <div className="flex flex-col items-center justify-center">
                  <div className="flex items-center justify-center rounded-lg bg-oxford/5 px-4 py-2 font-heading text-2xl font-700 text-oxford">
                    {fixture.status === "PLAYED" ? (
                      <span className="tracking-widest">{fixture.ourScore ?? '-'} <span className="text-muted-foreground text-sm font-400 mx-1">Vs</span> {fixture.theirScore ?? '-'}</span>
                    ) : (
                ...
                <div className="flex flex-1 flex-col items-center gap-2">
                  <div className="relative flex h-16 w-16 items-center justify-center rounded-full bg-white p-2 shadow-sm">
                    <Image src={awayTeamAvatar} alt={fixture.awayTeam.name} fill className="object-contain p-1" />
                  </div>
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
        <FixtureCard discipline="Básquetbol" title="Básquetbol" colorClass="bg-orange-500" fixtures={initialFixtures} />
        <FixtureCard discipline="Voleibol" title="Voleibol" colorClass="bg-yellow-400 text-oxford" fixtures={initialFixtures} />
      </div>
    </section>
  );
}
