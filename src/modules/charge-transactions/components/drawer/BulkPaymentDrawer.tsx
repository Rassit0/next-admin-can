...
export const BulkPaymentDrawer = ({ isOpen, onOpenChange, charges, payerPerson, onSuccess, onError }: Props) => {
  const router = useRouter();
  
  const totalAmount = charges.reduce((sum, charge) => sum + Number(charge.pendingAmount || 0), 0);
  ...
  const validate = () => {
    ...
    if (charges.length === 0) newErrors.general = "No hay cargos seleccionados.";
    ...
    if (hasUncustomizedCharges) {
      if (!paymentMethod) newErrors.paymentMethod = "Seleccione un método de pago global por defecto";
      if (!financialAccountId) newErrors.financialAccountId = "Seleccione una cuenta financiera global por defecto";
    ...
    for (const charge of charges) {
        ...
        if (splits) {
            const sum = splits.reduce((acc, curr) => acc + Number(curr.amount || 0), 0);
            if (Number(sum.toFixed(2)) !== Number(Number(charge.pendingAmount).toFixed(2))) {
                newErrors[`charge_${charge.id}`] = "La suma de la distribución no coincide con el saldo del cargo.";
            }
            const hasInvalidMethod = splits.some((s) => !s.paymentMethod || !s.financialAccountId || Number(s.amount) <= 0);
            if (hasInvalidMethod) {
                newErrors[`charge_${charge.id}`] = "La distribución tiene cuentas, métodos o montos inválidos.";
            }
        }
    }
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    try {
      const chargeItems = charges.map(c => {
         ...
         return {
            chargeId: c.id,
            splitTransactions: splits ? splits.map(s => ({
      ...
      if (res.error) {
        toast.danger(res.message || "Es posible que el saldo de los cargos haya cambiado.");
        if (onError) onError();
      } else {
        toast.success(res.message || "Los cargos han sido pagados exitosamente.");
    ...
    <>
      <Drawer.Backdrop isOpen={isOpen} onOpenChange={onOpenChange}>
        <Drawer.Content placement="right">
          <Drawer.Dialog className="w-full sm:max-w-md" aria-label="Cobrar Múltiples Cargos">
            <Drawer.CloseTrigger />
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <Select
                    ...
                    onSelectionChange={(key) => {
                      const val = key as "CASH" | "TRANSFER" | "QR";
                  ...
                  >
                    ...
                    <Select.Popover>
                      <ListBox>
                        <ListBox.Item id="CASH">Efectivo</ListBox.Item>
                        <ListBox.Item id="TRANSFER">Transferencia</ListBox.Item>
                        <ListBox.Item id="QR">Código QR</ListBox.Item>
                      </ListBox>
                    </Select.Popover>
                  </Select>
                  ...
                  <Select
                    ...
                    onSelectionChange={(key) => {
                      ...
                      if (val) setFinancialAccountId(val);
                    }}
                  ...
                  <div className="p-4 flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-4 max-h-80 overflow-y-auto pr-2">
                      {charges.map((charge) => {
                        ...
                        return (
                        <div key={charge.id} className="flex flex-col gap-2 border-b border-default-100 pb-2">
                            <div className="flex justify-between items-center text-sm">
                              <span className="text-default-700 line-clamp-1 flex-1 pr-2" title={charge.description}>
                                {charge.description}
                              </span>
                              <span className="font-medium text-right shrink-0">
                                Bs {formatCurrency(Number(charge.pendingAmount || 0))}
                              </span>
                            </div>
                            
                            <div className="flex justify-between items-center text-xs">
                              {isCustom ? (
                                 <span className="text-primary-600 font-semibold bg-primary-50 px-2 py-0.5 rounded-full border border-primary-200">Personalizado ({customSplits[charge.id].length} split{customSplits[charge.id].length !== 1 && 's'})</span>
                              ) : (
                                 <span className="text-default-400">Usa default global</span>
                              )}
                                ...
                                onPress={() => {
                                    if (isExpanded) {
                                        ...
                                        if (!customSplits[charge.id]) {
                                            // Initialize with 100%
                                            const defaultAcc = financialAccounts.find(a => a.id === financialAccountId) || financialAccounts.find((a) => a.isDefault);
                                            setCustomSplits(prev => ({
                                                ...prev,
                                                [charge.id]: [{
                                                    id: Math.random().toString(),
                                                    amount: Number(charge.pendingAmount).toString(),
                                                    paymentMethod: paymentMethod || defaultAcc?.allowedPaymentMethods?.[0] || "",
                                                    financialAccountId: financialAccountId || defaultAcc?.id || "",
                                                    reference: ""
                                                }]
                                            }));
                                        }
                              ...
                              >
                                {isExpanded ? "Ocultar" : (isCustom ? "Editar" : "Personalizar")}
                              </Button>
                            </div>
                            
                            {errors[`charge_${charge.id}`] && (
                                <p className="text-xs text-danger-500 font-medium">{errors[`charge_${charge.id}`]}</p>
                            )}
