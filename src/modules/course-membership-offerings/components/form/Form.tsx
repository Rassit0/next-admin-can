...
export const FormCourseOffering = ({ course, courseSeason, urlRedirect }: Props) => {
  ...
  // form params
  const [name, setName] = useState<string | null>(courseSeason?.name || null);
  const [startDate, setStartDate] = useState<DateValue | null>(
    courseSeason?.startDate
      ? parseDate(
          `${courseSeason.startDate.getFullYear()}-${(courseSeason.startDate.getMonth() + 1).toString().padStart(2, "0")}-${courseSeason.startDate.getDate().toString().padStart(2, "0")}`,
        )
      : null,
  );
  const [endDate, setEndDate] = useState<DateValue | null>(
    courseSeason?.endDate
      ? parseDate(
          `${courseSeason.endDate.getFullYear()}-${(courseSeason.endDate.getMonth() + 1).toString().padStart(2, "0")}-${courseSeason.endDate.getDate().toString().padStart(2, "0")}`,
        )
      : null,
  );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if ((e.target as HTMLFormElement).id !== "form-course-offering") {
      return;
    }
