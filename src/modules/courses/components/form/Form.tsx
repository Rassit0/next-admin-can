...
import { addCourse, editCourse, ICourse } from "@/modules/courses";
...
export const FormCourse = ({
  ...
  const [name, setName] = useState(course?.name || null);
  const [shortName, setShortName] = useState(course?.shortName || null);
  const [description, setDescription] = useState(course?.description || null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (course) {
      res = await editCourse({ id: course.id, data });
    } else {
