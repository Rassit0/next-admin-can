...
export const EnrollMembershipForm = ({
  ...
  const selectedCategoryData = categories.find((c) => c.id === categoryKey);
        ...
        <Surface variant="transparent" className="flex flex-col gap-5 mt-2">
          ...
          >
            <Label className="text-sm font-semibold flex items-center">
              ...
              {selectedCategoryData ? (
                <InfoTooltip
                  text={`Categorí­a: ${selectedCategoryData.category.name}. Edades: ${selectedCategoryData.category.minAge} a ${selectedCategoryData.category.maxAge || "Sin lmite"} aos. Temporada: ${new Date(teamSeason.season.startDate).toLocaleDateString()} - ${new Date(teamSeason.season.endDate).toLocaleDateString()}.`}
                />
              ) : (
                <InfoTooltip text="La categora en la que el jugador ser inscrito." />
              )}
            </Label>
            <Select.Trigger>
              <Select.Value />
          ...
          {breakdown?.data && (
            <InvoicePreview
              ...
              playerName={selectedPerson ? selectedPerson.fullName : null}
            />
          )}
        </Surface>
      </Drawer.Body>
