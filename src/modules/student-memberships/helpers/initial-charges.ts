...
/**
 * Computes the initial charges generated when a student is enrolled into a
 * CourseSeason. The base prices come 100% from the CourseSeason configuration
...
export const calculateInitialCharges = (
  ...
  const registrationGross = toNumber(courseSeason.billingConfig?.registrationFee);
  const monthlyGross = toNumber(courseSeason.billingConfig?.recurringFee);
