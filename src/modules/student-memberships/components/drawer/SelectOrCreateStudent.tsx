...
interface Props {
  ...
  setSelectedStudent: Dispatch<SetStateAction<IStudentOption | null>>;
...
export const SelectOrCreateStudent = ({
  isRequired = true,
  isDisabled = false,
  label,
  studentId,
  ...
  const list = useAsyncList<IStudentOption>({
    async load({ cursor: page = "1", filterText, signal }) {
      const res = await getStudentsOptions({ search: filterText, page });
      console.log({ res });
      if (!res) {
        return {
          cursor: undefined,
  ...
  return (
    <div className="flex items-end gap-4 w-full">
      ...
      >
        ...
        <FieldError children={errors?.studentId && <p>{errors.studentId}</p>} />
      </Autocomplete>
      <AddModal
        isIcon
        onSubmited={() => {
