...
import { StatusChip as PlayerStatusChip, PlayerMembershipStatus } from "@/modules/player-memberships";
import { StatusChip as StudentStatusChip, StudentMembershipStatus } from "@/modules/student-memberships";
  ...
  return (
    <Drawer.Backdrop isOpen={isOpen} onOpenChange={onOpenChange}>
      <Drawer.Content placement="right">
        <Drawer.Dialog aria-label="Historial de Membresías" className="w-full sm:max-w-md overflow-y-hidden flex flex-col h-full bg-surface">
          <Drawer.CloseTrigger />
          <Drawer.Header className="border-b border-border shrink-0">
            <Drawer.Heading className="text-xl font-bold flex items-center gap-2">
              <i className="ri-history-line text-default-500"></i> Historial de Membresías
            </Drawer.Heading>
            ...
            <div className="absolute inset-0 flex flex-col h-full bg-surface-secondary/20 p-4 overflow-y-auto">
              {isLoading && history.length === 0 ? (
                ...
                <div className="flex flex-col gap-4">
                  {history.map((item) => (
                    <div key={item.id} className="bg-surface border border-border rounded-lg p-4 flex flex-col gap-2 relative shadow-sm">
                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-2 text-sm font-semibold text-default-600 mb-1">
                          {item.type === "team" ? (
                            <><i className="ri-trophy-line text-primary"></i> Equipo</>
                          ) : (
                            <><i className="ri-graduation-cap-line text-secondary"></i> Curso</>
                          )}
                        </div>
                        {item.type === "team" ? (
                           <PlayerStatusChip status={item.status as PlayerMembershipStatus} />
                        ) : (
                           <StudentStatusChip status={item.status as StudentMembershipStatus} />
                        )}
                      </div>

                      {item.type === "team" ? (
                        <>
                          {item.teamName && <p className="font-semibold text-foreground">{item.teamName}</p>}
                          {item.seasonName && <p className="text-sm text-default-600">{item.seasonName}</p>}
                          {item.categoryName && <p className="text-sm text-default-500">Categor�a {item.categoryName}</p>}
                        </>
                      ) : (
                        <>
                          {item.courseName && <p className="font-semibold text-foreground">{item.courseName}</p>}
                          {item.seasonName && <p className="text-sm text-default-600">{item.seasonName}</p>}
                          {item.shiftName && <p className="text-sm text-default-500">Turno: {item.shiftName}</p>}
                        </>
                      )}

                      <div className="mt-2 text-xs text-default-500 flex flex-col gap-1 border-t border-border pt-2">
                        {item.startedAt && <p>Inicio: {formatDate(item.startedAt)}</p>}
