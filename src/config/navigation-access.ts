...
export const hasModuleAccess = (
  ...
  if (moduleId === "web") {
    ...
    const childRoute = itemsWebNavigation.find(
      (r: any) => r.id === childRouteId,
    );
    if (!childRoute) return false;

    if (childRoute.requiredPermissions) {
      if (
        !hasRequiredPermissions(userPermissions, childRoute.requiredPermissions)
      ) {
        return false;
      }
    }
  ...
  if (moduleItem.requiredPermissions) {
    if (
      !hasRequiredPermissions(userPermissions, moduleItem.requiredPermissions)
    ) {
      return false;
    }
  }
