...
## ?? Reglas Cr�ticas de Prisma

- **NUNCA usar `npx prisma db push`** en este ecosistema.
- **SIEMPRE usar `npx prisma migrate dev`** al desarrollar cambios en el esquema.
- *Raz�n:* `db push` no crea archivos de migraci�n y causa desincronizaci�n (Drift) con el esquema real de la base de datos, lo cual genera conflictos en el entorno de producci�n que s� usa migraciones.
